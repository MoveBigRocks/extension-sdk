// Package bundletrust defines the canonical signed-extension bundle format.
package bundletrust
...
type trustEnvelope struct {
	...
	License   LicenseClaim `json:"license"`
}
...
func NewVerifier(instanceID string, requireVerification bool, trustedPublishers map[string]map[string]string) (*Verifier, error) {
...
func (v *Verifier) VerifyBundle(_ context.Context, manifest ManifestIdentity, licenseToken string, bundle []byte) error {
	if v == nil {
		return nil
	}
	publisherKeys := v.trustedPublisherKeys[strings.TrimSpace(manifest.Publisher)]
	if !v.requireVerification && len(publisherKeys) == 0 {
		return nil
	}
...
func CanonicalSignedBundlePayload(manifestRaw, assetsRaw, migrationsRaw json.RawMessage, license LicenseClaim) ([]byte, error) {
	manifestValue, err := decodeSection(manifestRaw, map[string]any{})
	if err != nil {
		return nil, fmt.Errorf("decode manifest: %w", err)
	}
	assetsValue, err := decodeSection(assetsRaw, []any{})
	if err != nil {
		return nil, fmt.Errorf("decode assets: %w", err)
	}
	migrationsValue, err := decodeSection(migrationsRaw, []any{})
	if err != nil {
		return nil, fmt.Errorf("decode migrations: %w", err)
	}
	return json.Marshal(map[string]any{
		"assets": assetsValue, "license": license, "manifest": manifestValue, "migrations": migrationsValue,
	})
}
...
func decodeSection(raw json.RawMessage, defaultValue any) (any, error) {
	if len(raw) == 0 {
